import { creerStripe, lireDroits, PRIX, verifierPrix, verifierSignature } from './stripe.ts';
import type { Objet, Offre, Stripe } from './stripe.ts';

export type Configuration = { supabase: string; service: string; stripe: string; signature: string; site: string; testeurs: string[] };
...
export function creerPaiements(config: Configuration, requete: typeof fetch = fetch) {
  const stripe: Stripe = creerStripe(config.stripe, requete);
  ...
  const cors = { 'Access-Control-Allow-Origin': site.origin, 'Access-Control-Allow-Headers': 'authorization, apikey, content-type', 'Access-Control-Allow-Methods': 'POST, OPTIONS', Vary: 'Origin' };
  ...
  const rpc = (nom: string, body: unknown) => db(`rpc/${nom}`, body);
  ...
  async function synchroniser(l: Liaison) {
    ...
    const droits = await lireDroits(stripe, l.client_stripe, l.id);
  ...
  async function paiement(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (!config.testeurs.includes(user.id)) return json({ erreur: 'Les paiements sont réservés aux comptes de test autorisés.' }, 403);
      ...
      if (body.action === 'achat' && (!comptes[0].code_defini_le || !comptes[0].annee_de_naissance || new Date().getUTCFullYear() - comptes[0].annee_de_naissance < 18)) {
        return json({ erreur: 'Déclare ton âge et crée ton code de secours dans les Réglages avant le test.' }, 403);
      }
      const liaison = await rpc('preparer_paiement_test', { p_utilisateur: user.id }) as Liaison;
      return await sousVerrou(liaison.id, async l => {
        if (!l.client_stripe) {
          ...
          const customer = await stripe('customers', { 'metadata[compte]': l.id, 'metadata[application]': 'philamots-test' }, `client-${l.id}`);
        ...
        if (body.action === 'portail') {
          const configuration = await stripe('billing_portal/configurations', {
            'business_profile[headline]': 'Philamots — environnement de test',
        ...
        verifierPrix(await stripe(`prices/${PRIX[offre].id}`), offre);
        ...
        const mode = offre === 'necessaire' ? 'payment' : 'subscription';
        const prefixe = mode === 'payment' ? 'payment_intent_data' : 'subscription_data';
        const session = await stripe('checkout/sessions', {
          mode, customer: l.client_stripe!, client_reference_id: l.id,
  ...
  async function webhook(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (evt.livemode !== false) return json({ erreur: 'Événement réel interdit.' }, 400);
      ...
      if (customer.metadata?.application !== 'philamots-test') return json({ ignore: true });
      const liens = await db(`paiements_test?id=eq.${encodeURIComponent(customer.metadata.compte)}&select=*`);
