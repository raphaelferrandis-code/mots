...
export function InviterDesAmis({ grand = false }: { grand?: boolean }) {
  ...
  return cadre(<>
    ...
    <div className="par-avion__bilan">
      <div className="par-avion__compte"><span className="mention">Amis récompensés</span><span><strong>{p.valides}</strong> · {limite} par mois au plus</span></div>
      <div className="jauge-parrainage" aria-hidden="true">{Array.from({ length: limite }, (_, i) => <i key={i} className={i < p.valides ? 'allume' : undefined} />)}</div>
      <p>{bilan}</p>
    </div>
  </>);
}
