...
// d'obtention (cachet). La définition n'est plus imprimée sur le timbre : elle se lit
...
export const Timbre = memo(function Timbre({ carte, finition = 'Normale', oblitere = false, obtenuLe = null, verso = false, montrerVerso = false, dosRenseigne = false, dos = 'gomme', dentele = true, reagir, cliquable = true, onChoisir, action, className, style }: Props) {
  ...
  const description = `${carte.mot}, ${carte.type}, ${carte.rarete}${finition === 'Normale' || horsSerie ? '' : `, finition ${finition.toLowerCase()}`}, ${carte.faction}, attaque ${attaque}, défense ${defense}`;
  const commun = {
