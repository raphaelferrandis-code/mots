...
// Un seul dessin, découpé en deux morceaux complémentaires le long du bord déchiré.
// Les exemplaires hors de la scène restent parfaitement assemblés.
export function PaquetScelle({ modele }: { modele?: string } = {}) {
  ...
  const id = useId();
  const dechirure = 'M22 103l12-5 9 8 13-5 10 7 12-9 11 7 13-4 9 8 12-10 13 5 10-6 12 9 11-5 13 4 9-8 12 6 12-3 10 7 13-9 11 5 12-7 12 5 13-3 12 4';
  return (
    <svg className="paquet-scelle" viewBox="0 0 320 440" aria-hidden="true" focusable="false">
      ...
      <use className="paquet-scelle__intact" href={`#${id}-dessin`} />
      <g className="paquet-scelle__corps">
        <path className="paquet-scelle__interieur" d="M23 104Q160 49 297 104L287 166H33Z" fill={`url(#${id}-interieur)`} stroke={design.metal} strokeOpacity=".65" />
        <use href={`#${id}-dessin`} clipPath={`url(#${id}-bas)`} />
        <path className="paquet-scelle__fibres" d={dechirure} fill="none" stroke="#f4e5c5" strokeWidth="2.5" />
      </g>
      <g className="paquet-scelle__haut">
        <use href={`#${id}-dessin`} clipPath={`url(#${id}-haut)`} />
        <path className="paquet-scelle__fibres" d={dechirure} fill="none" stroke="#f4e5c5" strokeWidth="2" />
      </g>
      <path className="paquet-scelle__dechirure" d={dechirure} fill="none" stroke="#fff1cf" strokeWidth="4" />
    </svg>
  );
}
