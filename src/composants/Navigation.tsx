...
// Sur téléphone, la barre du bas n'a que cinq onglets : le deck et les classements se rangent dans Duel
// (ordinateurSeulement), et l'onglet Duel reste allumé sur leurs pages (rubriqueSurTelephone).
type Onglet = { route: Route; nom: string; icone: ReactNode; actifPour: Route['ecran'][]; ordinateurSeulement?: boolean; rubriqueSurTelephone?: Route['ecran'][] };
...
const ONGLETS: Onglet[] = [
  ...
  { route: { ecran: 'deck' }, nom: 'Deck', actifPour: ['deck'], ordinateurSeulement: true, icone: <svg viewBox="0 0 24 24" {...trait}><path d="m12 4 8 4-8 4-8-4z" /><path d="m4 12 8 4 8-4" /><path d="m4 16 8 4 8-4" /></svg> },
  { route: { ecran: 'duel' }, nom: 'Duel', actifPour: ['duel', 'joutes'], rubriqueSurTelephone: ['deck', 'classement'], icone: <svg viewBox="0 0 24 24" {...trait}><path d="m5 4 10 10" /><path d="m19 4-10 10" /><path d="m4 17 3 3" /><path d="m20 17-3 3" /><path d="m6 15 3 3" /><path d="m18 15-3 3" /></svg> },
  { route: { ecran: 'classement' }, nom: 'Classement', actifPour: ['classement'], ordinateurSeulement: true, icone: <svg viewBox="0 0 24 24" {...trait}><path d="M3 20V11h6v9M9 20V5h6v15M15 20v-6h6v6M2 20h20" /></svg> },
    ...
    <header className="entete-application">
      ...
      <nav className="navigation" aria-label="Navigation principale">
        <ul className="navigation__liste">
          {ONGLETS.map((onglet) => (
            <li key={onglet.nom} className={onglet.ordinateurSeulement ? 'navigation__ordinateur' : undefined}>
              <a className={`navigation__lien${onglet.route.ecran === 'profil' ? ' lien-profil' : ''}`} href={lien(onglet.route)} aria-label={onglet.nom} aria-current={onglet.actifPour.includes(ecran) ? 'page' : undefined}
                data-rubrique={onglet.rubriqueSurTelephone?.includes(ecran) || undefined}>
