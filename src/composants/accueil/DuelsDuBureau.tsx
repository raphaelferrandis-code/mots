// Le bloc Duels de l'accueil (maquette) : une phrase qui dit où en est le joueur, la jauge des dix timbres du deck,
...
export function DuelsDuBureau({ sauvegarde, deck }: { sauvegarde: Sauvegarde; deck: number | null }) {
  ...
  const texte = complet
    ? joues === 0 ? 'Ton deck est prêt. Lance ton premier défi à un autre collectionneur.' : `${gagnes} victoire${gagnes > 1 ? 's' : ''} en ${joues} duel${joues > 1 ? 's' : ''}. Ton deck t’attend pour le suivant.`
    : jauge === 0 ? `Il te faut ${TAILLE === 10 ? 'dix' : TAILLE} timbres pour composer ton premier deck et défier un autre collectionneur.`
      : manque > 0 ? `Encore ${manque} timbre${manque > 1 ? 's' : ''} à trouver avant ton premier duel.`
        : `Tu as de quoi composer un deck. Choisis tes ${TAILLE === 10 ? 'dix' : TAILLE} meilleurs mots et lance un défi.`;
  ...
  return (
    <section className="duels-bureau" aria-labelledby="titre-duels-bureau">
      ...
      <p className="duels-bureau__compte">{complet ? <><b>{TAILLE}</b> sur {TAILLE} timbres dans ton deck</> : <><b>{jauge}</b> sur {TAILLE} timbres pour ton premier deck</>}</p>
      <p className="duels-bureau__liens">
        <a className="comptoir__lien" href={lien({ ecran: complet ? 'duel' : 'deck' })}>{complet ? 'Lancer un duel' : 'Composer mon deck'}</a>
