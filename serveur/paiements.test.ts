...
it('Stripe : droits issus des paiements, renouvellement, refus, résiliation et remboursement', async () => {
  ...
  const stripe: Stripe = async path => {
    ...
    if (p.pathname === '/checkout/sessions') return result([{ id: 'cs_album', livemode: false, metadata: { compte: 'compte', offre: 'necessaire' }, mode: 'payment', status: 'complete', payment_status: 'paid', payment_intent: 'pi_album' }]);
    ...
    if (p.pathname === '/charges') {
      ...
      return result([{ livemode: false, customer: 'cus_test', paid: true, status: 'succeeded', amount: album ? 599 : 499, amount_refunded: album ? rembourseAlbum : rembourseAbonnement, disputed: litige }]);
    }
    if (p.pathname === '/subscriptions') return result([{ id: 'sub_test', livemode: false, metadata: { compte: 'compte', offre: 'collectionneur' }, status: finAbonnement ? 'canceled' : 'active', ended_at: finAbonnement }]);
    if (p.pathname === '/invoices') return result(facturePayee ? [{ id: 'in_test', livemode: false, customer: 'cus_test', paid: true, amount_paid: 499, payment_intent: 'pi_sub' }] : []);
  ...
  const droits = () => lireDroits(stripe, 'cus_test', 'compte');
...
it('HTTP : crée un seul Checkout, ignore les prix fournis et n’accorde les droits qu’après confirmation', async () => {
  ...
  const network: typeof fetch = async (input, options) => {
    ...
    if (url.pathname === '/v1/checkout/sessions' && options?.method === 'POST') {
      ...
      return Response.json({ id: 'cs_test', url: 'https://checkout.stripe.com/test', livemode: false });
    }
    if (url.pathname === '/v1/checkout/sessions/cs_test') return Response.json({ status: 'open', url: 'https://checkout.stripe.com/test' });
    if (url.pathname === '/v1/customers/cus_test') return Response.json({ metadata: { application: 'philamots-test', compte: 'compte' }, livemode: false });
    let data: Objet[] = [];
    if (paye && url.pathname === '/v1/checkout/sessions') data = [{ id: 'cs_test', livemode: false, metadata: { compte: 'compte', offre: 'necessaire' }, mode: 'payment', status: 'complete', payment_status: 'paid', payment_intent: 'pi_test' }];
    if (url.pathname === '/v1/checkout/sessions/cs_test/line_items') data = [{ price: { id: PRIX.necessaire.id }, quantity: 1 }];
    if (url.pathname === '/v1/charges') data = [{ livemode: false, customer: 'cus_test', paid: true, status: 'succeeded', amount: 599, amount_refunded: 0 }];
    return Response.json({ data, has_more: false });
  };
  const app = creerPaiements({ supabase: 'https://supabase.invalid', service: 'service', stripe: 'sk_test_factice', signature: 'whsec_test', site: 'https://philamots.fr', testeurs: [id] }, network);
  ...
  paye = true;
  const event = JSON.stringify({ id: 'evt_test', livemode: false, type: 'checkout.session.completed', data: { object: { customer: 'cus_test' } } });
