...
import { empreinte, rangs } from './stats.ts';
...
export function attribuerRaretes(mots: MotARarete[], reglages: ReglagesRarete): Map<string, Rarete> {
  ...
  return new Map([...classer(mesures, reglages), ...classer(nonMesures, reglages)]);
}

function classer(mots: MotARarete[], reglages: ReglagesRarete): Map<string, Rarete> {
  ...
  const raretes = new Map<string, Rarete>();
  let seuil = 0;
  let position = 0;
  for (const [rarete, part] of reglages.parts) {
    ...
    for (; position < fin; position++) raretes.set(ordre[position].id, rarete);
  }
  return raretes;
}
