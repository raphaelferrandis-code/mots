...
6. **Le simulateur de marché — fait le 22/09/2026 au soir** (`npm run simulation:marche`, rapport dans `data/simulation-marche.md`) : 60 joueurs fictifs pendant 120 jours, avec les vraies cartes, les vraies règles des paquets et des duels, et les vrais réglages du marché. Quatre enseignements :
   - **La commission de 10 % n'est pas ce qui tient l'économie** : elle ne détruit que 1 % de l'Encre qui disparaît, tout le reste part en paquets achetés. La doubler ne change presque rien. C'est un prix de service raisonnable, pas un levier. Le vrai levier reste le prix du paquet.
   - **Le plancher des timbres communs (5 Encre) bloque le marché quand les joueurs sont tièdes** : au « désir ×2 », 15 751 timbres communs proposés, 5 vendus. Des planchers au double de l'Encre d'un doublon (2 / 6 / 20 / 60 / 200) ramèneraient les invendus de 87 % à 33 %. Garder les planchers actuels est un choix défendable : cela évite un marché noyé sous les timbres communs.
   - **Le plancher des Légendaires (300) est le plus dur** : même au « désir ×5 », la moitié des Légendaires proposées restent invendues.
   - **Le plafond de 3 ventes en cours touche tout le monde** : 60 joueurs sur 60 y butent, parce qu'un joueur accumule des timbres en double finition bien plus vite que trois à la fois. Sans plafond, le marché voit cinq fois plus de ventes. À décider : monter le plafond (10, par exemple) ou l'assumer comme un argument de la version payante.

   Ce que le simulateur ne sait pas faire : deviner ce qu'un vrai joueur est prêt à payer. Il donne donc tous ses tableaux pour trois niveaux de « désir ». Les entrées et sorties d'Encre, elles, sont exactes.
7. Le **cachet de provenance** dessiné sur le timbre est laissé de côté (choix de Raphaël, 22/09/2026) ; la provenance reste enregistrée par le serveur.
