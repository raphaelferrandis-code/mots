...
*Le jeu s'est appelé « MOTS » jusqu'au 22 septembre 2026. Raphaël a choisi **Philamots** (philatélie et mots). `philamots.fr` est désormais actif chez OVHcloud ; le raccordement au site et la création de `contact@philamots.fr` restent à terminer : voir [le guide](docs/GUIDE-domaine-et-contact.md). Aucun achat du `.com` n’est confirmé. Le dépôt garde son nom `mots`.*

**Décision du 23 septembre :** l’Encre sert uniquement aux enchères. L’achat de paquets a été retiré du code local et du script serveur ; mise à jour de Supabase et publication restent à effectuer (voir docs/GUIDE-supabase.md). L’offre de lancement comprendra un achat unique et un seul abonnement, dont les avantages validés sont implémentés localement (docs/BRIEF-version-payante.md). Appliquer serveur/6-offres.sql avant publication. Les descriptions à trois formules ci-dessous concernent la version précédente.

- **Où en est le projet, et ce qui reste à faire : [ETAT-DU-PROJET.md](ETAT-DU-PROJET.md).** Commencer par là.
- Le projet est décrit dans [docs/BRIEF-v2.md](docs/BRIEF-v2.md) — c'est le document de référence.
- Le plan du serveur des joutes classées (duels contre d'autres joueurs) : [docs/BRIEF-joutes.md](docs/BRIEF-joutes.md) ; sa mise en route pas à pas : [docs/GUIDE-supabase.md](docs/GUIDE-supabase.md).
- Le programme des testeurs : [docs/GUIDE-testeurs.md](docs/GUIDE-testeurs.md), et leurs retours : [docs/RETOURS-testeurs.md](docs/RETOURS-testeurs.md).
- Le plan du marché (échanges et enchères entre joueurs) : [docs/BRIEF-marche.md](docs/BRIEF-marche.md).
- Les amis, échanges directs et défis amicaux (déployés le 24 septembre 2026) : [docs/GUIDE-amis.md](docs/GUIDE-amis.md).
- Les équipes de deux joueurs (nom, emblème et invitations ; déployées le 24 septembre 2026) : [docs/GUIDE-equipes.md](docs/GUIDE-equipes.md).
- Ce qui bloque la version payante : [docs/BRIEF-version-payante.md](docs/BRIEF-version-payante.md), et le brouillon des conditions de vente : [docs/CGV-brouillon.md](docs/CGV-brouillon.md).
- L'origine et la licence des données sont dans [docs/SOURCES.md](docs/SOURCES.md).
- Ce que contiennent vraiment les données : [docs/COMPTE-RENDU-donnees.md](docs/COMPTE-RENDU-donnees.md).
