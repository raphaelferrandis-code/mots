...
| `src/jeu/` | **Les règles du jeu**, sans écran ni stockage : tirage des paquets, recharge, Encre, sauvegarde, duel (`duel.ts`), épreuve de maîtrise (`epreuve.ts`), joutes classées (`joute.ts`), deck, maîtrise et récompenses (`progression.ts`), enchères du marché (`marche.ts`). Entièrement couvertes par des tests |
