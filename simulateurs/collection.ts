...
function simulerUnJoueur(paquetsParJour: number, graine: number, equilibrage: Equilibrage): Bilan {
  ...
  const { paquets, encreParDoublon, finitions } = equilibrage;
  ...
  let ordinaires = 0, encre = 0, ouverts = 0, achetes = 0, sansLegendaire = 0, legendairesDesHuitPremieresSemaines = 0;
  ...
  const ouvrir = (jour: number): void => {
    ...
    for (const { carte, finition } of tirees) {
      ...
      else encre += encreParDoublon[carte.rarete] * finitions.encre[finition];
    }
  };
  ...
  return { joursParPalier, legendairesParSemaine: legendairesDesHuitPremieresSemaines / 8, partDePaquetsAchetes: achetes / ouverts, premiereHorsSerie, horsSerieEnUnAn, finitionsRaresEnUnMois };
}
...
const retour = encreMaximaleMoyenneParPaquet(EQUILIBRAGE.paquets, EQUILIBRAGE.finitions, EQUILIBRAGE.encreParDoublon);
const rapport = [
  '# Simulation de collection',
  '',
  `*Généré par \`npm run simulation:collection\`. Édition de ${TOTAL.toLocaleString('fr-FR')} cartes ordinaires dont ${LEGENDAIRES} Légendaires, plus ${HORS_SERIE} cartes Hors-série comptées à part ; ${JOUEURS_PAR_PROFIL} joueurs simulés par ligne (on retient le joueur du milieu) ; chaque joueur dépense son Encre en paquets dès qu'il le peut. Les réglages sont dans \`src/config/equilibrage.ts\`.*`,
  '',
